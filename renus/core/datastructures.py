...
class Store(object):
    """
    An object that can be used to store arbitrary store.

    Used for `request.store` and `ws.store`.
    """

    def __init__(self, store: typing.Dict = None):
        if store is None:
            store = {}
        super(Store, self).__setattr__("_store", store)
